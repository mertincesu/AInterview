    ...
    methods: {
      ...
      async saveCompletedQuestion() {
        ...
        if (this.completedQuestions.length >= 5) {
          this.completed = true;
          await this.generateFeedbackForAllQuestions();
        } else {
          this.nextQuestion();
        }
      },
      async generateFeedbackForAllQuestions() {
        for (let i = 0; i < this.completedQuestions.length; i++) {
          const question = this.completedQuestions[i].question;
          const transcript = this.completedQuestions[i].response;
          const feedback = await this.getFeedback(question, transcript);
          this.completedQuestions[i].feedback = feedback;
        }
...
.interview-container {
  ...
  padding-bottom: 20px; /* Ensure 20px extra padding at the bottom */
...
.card-container {
  width: 90%;
  max-width: 1600px;
  max-height: calc(100vh - 160px); /* Ensures the card does not exceed the height of the navbar */
  padding: 20px;
  background: #ffffff;
  border-radius: 10px;
  ...
  overflow-y: auto; /* Ensure card content can scroll if needed */
}
...
/* Completed question section styles */
.completed-question {
  background: #f9f9f9;
  padding: 20px;
  border-radius: 8px;
...
/* Button styles */
.next-button, .record-button, .start-button {
  width: 15%;
  padding: 15px 25px;
  font-size: 20px;
  font-weight: bold;
